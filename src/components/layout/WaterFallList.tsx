...
export default class WaterfallList<
  T extends Record<string, any>,
> extends Component<WaterfallListProps<T>, WaterfallListState<T>> {
  static defauleProps = {
    scrollEventThrottle: 50,
    numColumns: 1,
  };

  state = _reconstructData<T>(this.props as any);
  _listRefs: Array<VirtualizedList<T> | null> = [];
  _scrollRef: ScrollView | null | undefined;

  UNSAFE_componentWillReceiveProps(
    nextProps: Readonly<WaterfallListProps<T>>,
  ): void {
    this.setState(_reconstructData<T>(nextProps as any));
  }
  ...
  //       list._onContentSizeChange(width, heihgt),
  //   );
  // };
  ...
  _onScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    // this._listRefs.forEach(list => list?._onScroll && list._onScroll(event));
  };
...
function _reconstructData<ItemT = any>(params: {
  numColumns: number;
  data: Array<any>;
